...
var U = /\{[^{}]+\}/g, y = ({ allowReserved: s, name: e, value: a }) => {
  if (a == null) return "";
  ...
  if (!e) {
    let r = (s ? l : l.map((u) => encodeURIComponent(u))).join(W(i));
    switch (i) {
      case "label":
        return `.${r}`;
      case "matrix":
  ...
  let n = _(i), t = Object.entries(l).map(([r, u]) => y({ allowReserved: s, name: i === "deepObject" ? `${a}[${r}]` : r, value: u })).join(n);
  return i === "label" || i === "matrix" ? n + t : t;
}, E = ({ path: s, url: e }) => {
  let a = e, i = e.match(U);
  if (i) for (let l of i) {
  ...
  return a;
}, S = ({ allowReserved: s, array: e, object: a } = {}) => (i) => {
  let l = [];
  ...
  var i;
  let a = { ...s, ...e };
  return (i = a.baseUrl) != null && i.endsWith("/") && (a.baseUrl = a.baseUrl.substring(0, a.baseUrl.length - 1)), a.headers = O(s.headers, e.headers), a;
}, O = (...s) => {
  let e = new Headers();
  for (let a of s) {
...
}, D = () => ({ error: new v(), request: new v(), response: new v() }), N = { bodySerializer: (s) => JSON.stringify(s) }, P = S({ allowReserved: !1, array: { explode: !0, style: "form" }, object: { explode: !0, style: "deepObject" } }), H = { "Content-Type": "application/json" }, T = (s = {}) => ({ ...N, baseUrl: "", fetch: globalThis.fetch, headers: H, parseAs: "auto", querySerializer: P, ...s }), J = (s = {}) => {
  let e = j(T(), s), a = () => ({ ...e }), i = (t) => (e = j(e, t), a()), l = D(), n = async (t) => {
    let r = { ...e, ...t, headers: O(e.headers, t.headers) };
    r.body && r.bodySerializer && (r.body = r.bodySerializer(r.body)), r.body || r.headers.delete("Content-Type");
    let u = I({ baseUrl: r.baseUrl ?? "", path: r.path, query: r.query, querySerializer: typeof r.querySerializer == "function" ? r.querySerializer : S(r.querySerializer), url: r.url }), d = { redirect: "follow", ...r }, c = new Request(u, d);
...
class L {
  ...
  static whatsMyName(e) {
    return ((e == null ? void 0 : e.client) ?? b).get({
      ...e,
      url: "/umbraco/wscumbcare/api/v1/whatsMyName"
    });
  }
  static whatsTheTimeMrWolf(e) {
    return ((e == null ? void 0 : e.client) ?? b).get({
      ...e,
      url: "/umbraco/wscumbcare/api/v1/whatsTheTimeMrWolf"
    });
  }
  static whoAmI(e) {
    return ((e == null ? void 0 : e.client) ?? b).get({
      ...e,
      url: "/umbraco/wscumbcare/api/v1/whoAmI"
    });
  }
}
export {
...
//# sourceMappingURL=services.gen-CH-rdfn7.js.map
